#!/usr/bin/env python3
"""
Merge per-run HDF5 files into per-year and all-years files.

Intermediate files are written next to the run files:
...
def parse_args():
    ...
    p.add_argument("--input-dir", default="/data/user/tvaneede/muon_tagging/h5",
                   help="Root directory of per-run HDF5 files")
...
def main():
    ...
    for year in sorted(args.years):
        ...
        for run_dir in sorted(glob.glob(os.path.join(year_dir, "Run*"))):
            ...
            if not h5_files:
                print(f"  WARNING: no h5 file in {run_dir}")
                continue
            if len(h5_files) > 1:
                print(f"  WARNING: multiple h5 files in {run_dir}, expected one per run")
            run_files.append(h5_files[0])
        ...
        print(f"IC86_{year}: merging {len(run_files)} run(s) → {year_out}")
